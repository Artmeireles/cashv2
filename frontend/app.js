...
const configs = {
    ...
    // forcarHTTP: true, //Defina para true se desejar que o redirecionamento para HTTP seja forçado
...
if (configs.forcarHTTPS) //Se o redirecionamento HTTP estiver habilitado, registra o middleware abaixo
    ...
    // else if (configs.forcarHTTP) //Se o redirecionamento HTTP estiver habilitado, registra o middleware abaixo
    //     app.use((req, res, next) => { //Cria um middleware onde todas as requests passam por ele
    //         if (req.headers["x-forwarded-proto"] == "https") //Checa se o protocolo informado nos headers é HTTP
    //             res.redirect(`http://${req.headers.host}${req.url}`); //Redireciona pra HTTPS
    //         else //Se a requisição já é HTTPS 
    //             next(); //Não precisa redirecionar, passa para os próximos middlewares que servirão com o conteúdo desejado
    //     });

app.use(express.static(configs.caminho)); //Serve os outros arquivos, como CSSs, Javascripts, Imagens etc.
app.use(express.static(__dirname + configs.images)); //Serve os arquivos de imagens dos servidores.
