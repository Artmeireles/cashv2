...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            ...
            existsOrError(body.id_fin_eventosdesconto, 'Evento pertencente à base não informado')
        } catch (error) {
            app.api.logger.logError({ log: { line: `Error in file: ${__filename}.${__function} ${error}`, sConsole: true } })
    ...
    const saveByBatch = async (req, res) => {
        ...
        for (let obj in batch) {
            ...
            if (!body.id_fin_eventosdesconto) bodyValidation.push('Evento pertencente à base não informado')
            ...
            const bodyIdFinEventoDesconto = await app.db({ fe: tabelaEventosDomain }).where({ id_evento: body.id_fin_eventosdesconto }).first()
            const excluded = await app.db(tabelaDomain).where({ id_fin_eventos: bodyIdFinEvento.id, id_fin_eventosdesconto: bodyIdFinEventoDesconto.id }).delete()
            console.log('excluded:', 'bodyIdFinEvento', bodyIdFinEvento.id_evento, 'bodyIdFinEventoDesconto', bodyIdFinEventoDesconto.id_evento);
            body.id_fin_eventos = (bodyIdFinEvento && bodyIdFinEvento.id ? bodyIdFinEvento.id : null) || undefined
            body.id_fin_eventosdesconto = (bodyIdFinEventoDesconto && bodyIdFinEventoDesconto.id ? bodyIdFinEventoDesconto.id : null) || undefined
