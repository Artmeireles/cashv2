...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            ...
            existsOrError(body.id_eventos_percentual, 'ID do evento não informado')
            existsOrError(body.data, 'Data do evento não informado')
            existsOrError(body.percentual, 'Percentual do evento não informado')
        } catch (error) {
            app.api.logger.logError({ log: { line: `Error in file: ${__filename}.${__function} ${error}`, sConsole: true } })
    ...
    const saveByBatch = async (req, res) => {
        ...
        for (let obj in batch) {
            ...
            if (!body.id_eventos_percentual) bodyValidation.push('ID do evento não informado')
            if (!body.data) bodyValidation.push('Data do evento não informado')
            if (!body.percentual) bodyValidation.push('Percentual do evento não informado')
            ...
            const excluded = await app.db(tabelaDomain).where({ id_fin_eventos: bodyIdFinEvento.id, id_eventos_percentual: body.id_eventos_percentual }).delete()
            console.log('excluded:', 'bodyIdFinEvento', bodyIdFinEvento.id_evento, body.id_eventos_percentual);
            body.id_fin_eventos = (bodyIdFinEvento && bodyIdFinEvento.id ? bodyIdFinEvento.id : null) || undefineded

            if (bodyValidation.length > 0) {
                const mail = {
                    "from": "suporte@mgcash.app.br",
                    "to": "suporte@mgcash.app.br",
                    "subject": "Erro finEventosDesconto saveByBatch",
