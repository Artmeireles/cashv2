...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            existsOrError(body.id_pccs, 'PCCS não informado')
            existsOrError(body.classe, 'Classe não informada')
            existsOrError(body.i_ano_inicial, 'Ano Inicial não informado')
            existsOrError(body.i_ano_final, 'Ano Final não informado')
        }
        catch (error) {
    ...
    const get = async (req, res) => {
        ...
        let sql = app.db({ tbl1: tabelaDomain }).count('tbl1.id', { as: 'count' })
            ...
            .where(function () {
                this.where(app.db.raw(`tbl1.i_ano_inicial regexp('${key.toString().replace(' ', '.+')}')`))
                this.orWhere(app.db.raw(`tbl1.classe regexp('${key.toString().replace(' ', '.+')}')`))
            })
        ...
        ret.orderBy('classe').limit(limit).offset(page * limit - limit)
        ret.then(body => {
