...
module.exports = app => {
    ...
    const tabela = 'siafic_desapropriacao'
    ...
    const saveByBatch = async (req, res) => {
        ...
        try {
            ...
            isMatchOrError((uParams && (uParams.financeiro >= 3)), `${noAccessMsg} "Geração de dados do SIAFIC: Desapropriação"`)
        } catch (error) {
        ...
        try {
            ...
            existsOrError(body.dados, 'Dados da desapropriação não informados')
        } catch (error) {
        ...
        for (let obj in batch) {
            ...
            if (!bodyDados.desapropriacao_ferias) bodyValidation = { ...bodyValidation, 'Férias': 'Valor da desapropriação de férias não informado' }
            if (!bodyDados.desapropriacao_decimo) bodyValidation = { ...bodyValidation, 'Décimo': 'Valor da desapropriação do décimo não informado' }
