...
module.exports = app => {
    ...
    const get = async (req, res) => {
        ...
        if (statusV != undefined)
            sql.andWhere(app.db.raw(`tb1.vencimento ${statusV == '1' ? '>=' : '<'} DATE_SUB(NOW(),INTERVAL DAYOFMONTH(NOW())-1 DAY)`))
        ...
        if (statusV != undefined)
            ret.andWhere(app.db.raw(`tb1.vencimento ${statusV == '1' ? '>=' : '<'} DATE_SUB(NOW(),INTERVAL DAYOFMONTH(NOW())-1 DAY)`))
    ...
    const gPCE = async (req, res) => {
        ...
        try {
            // Alçada para edição de registro
            if (body.id)
                isMatchOrError(uParams && uParams.con_contratos >= 3, `${noAccessMsg} "Edição em massa de contrato de consignação"`)
            // Alçada para criação de registro
            else isMatchOrError(uParams && uParams.con_contratos >= 2, `${noAccessMsg} "Inclusão em massa de contrato de consignação"`)
        } catch (error) {
            return res.status(401).send(error)
        }
        const tabelaDomain = `${dbPrefix}_${uParams.cliente}_${uParams.dominio}.${tabela}`
        ...
        contratos.forEach(element => {
            ...
            for (let index = 1; index <= element.parcelas; index++) {
                const bodyParcela = {
                    ...
                    vencimento: moment(element.primeiro_vencimento).add(index - 1, 'month').format("YYYY-MM-DD"),
                    situacao: 1,
                }
                bodyParcelas.push(bodyParcela)
                linhasExecutadas++
        ...
        return res.status(200).send({ data: linhasExecutadas > 0 ? `${linhasExecutadas} parcelas incluídas com sucesso` : `Não foram localizados contratos sem parcelas` })
    }
