...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            existsOrError(body.pccs, 'PCCS não informado')
            existsOrError(body.nivel, 'Nível não informado')
    ...
    const get = async (req, res) => {
        ...
        let sql = app.db({ tbl1: tabelaDomain }).count('tbl1.id', { as: 'count' })
            ...
            .where(function () {
                this.where(app.db.raw(`tbl1.pccs regexp('${key.toString().replace(' ', '.+')}')`))
                this.orWhere(app.db.raw(`tbl1.nivel regexp('${key.toString().replace(' ', '.+')}')`))
            })
        ...
        ret.orderBy('pccs').limit(limit).offset(page * limit - limit)
        ret.then(body => {
