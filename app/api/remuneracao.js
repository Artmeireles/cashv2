...
module.exports = app => {
  ...
  const save = async (req, res) => {
    ...
    try {
      ...
      else isMatchOrError(uParams && uParams.cad_servidores >= 2, `${noAccessMsg} "Inclusão de ${tabela}"`)
    } catch (error) {
    ...
    for (const line of lines) {
      if (line.startsWith('INCLUIRS1207')) beneficiario = true
        ...
        if (servVinc && servVinc.id) {
          ...
          const vincUpdateBody = { 'id_param_cod_categ': codCateg, 'updated_at': new Date() }
          if (grauExp) vincUpdateBody.id_param_grau_exp = grauExp
          await app.db(tabelaServVinculoDomain).update(vincUpdateBody).where({ 'id': servVinc })
        } else {
  ...
  const get = async (req, res) => {
    ...
    try {
      ...
      isMatchOrError(uParams && uParams.cad_servidores >= 1, `${noAccessMsg} "Exibição de financeiros"`)
    } catch (error) {
      return res.status(401).send(error)
    }
    const tabelaDomain = `${dbPrefix}_${uParams.cliente}_${uParams.dominio}.${tabela}`
  ...
  const getById = async (req, res) => {
    ...
    try {
      // Alçada para exibição
      isMatchOrError(uParams && uParams.cad_servidores >= 1, `${noAccessMsg} "Exibição de cadastro de ${tabela}"`)
    } catch (error) {
      return res.status(401).send(error)
    }
  ...
  const remove = async (req, res) => {
    ...
    try {
      // Alçada para exibição
      isMatchOrError((uParams && uParams.cad_servidores >= 4), `${noAccessMsg} "Exclusão de cadastro de ${tabela}"`)
    } catch (error) {
      return res.status(401).send(error)
    }
    const tabelaDomain = `${dbPrefix}_${uParams.cliente}_${uParams.dominio}.${tabela}`
