...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            existsOrError(body.centro, 'Centro não informado')
            existsOrError(body.c_ua, 'Unidade Autononma não informada')
            existsOrError(body.cnpj_ua, 'CNPJ da Unidade Autonoma não informado')
            existsOrError(body.siap_data_criacao, 'Data de criação não informado')
            existsOrError(body.siap_data_ato, 'Data do ato de criação não informado')
            existsOrError(body.siap_ato, 'Número do ato de criação não informado')
            existsOrError(body.id_param_v_pub, 'Veículo de Publicação do ato não informado')
        }
        catch (error) {
    ...
    const get = async (req, res) => {
        ...
        let sql = app.db({ tbl1: tabelaDomain }).count('tbl1.id', { as: 'count' })
            ...
            .where(function () {
                this.where(app.db.raw(`tbl1.centro regexp('${key.toString().replace(' ', '.+')}')`))
                this.orWhere(app.db.raw(`tbl1.cnpj_ua regexp('${key.toString().replace(' ', '.+')}')`))
            })
        ...
        ret.orderBy('centro').limit(limit).offset(page * limit - limit)
        ret.then(body => {
