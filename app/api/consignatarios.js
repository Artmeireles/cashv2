...
module.exports = app => {
    ...
    const get = async(req, res) => {
        ...
        try {
            ...
            isMatchOrError(user.cpf == cpf || uParams.con_contratos >= 1 || uParams.gestor == 1 || uParams.admin >= 1, `${noAccessMsg} "Exibição de consignatários"`)
        } catch (error) {
            return res.status(401).send(error)
        }
        const tabelaDomain = `${dbPrefix}_${uParams.cliente}_${uParams.dominio}.${tabela}`
    ...
    const getConsignatarios = async(req, res) => {
        ...
        const ret = app.db({ td: `${tabelaDomain}` })
            .select(`td.id`, `cb.nome`, `cb.nomeAbrev`, `td.agencia`)
    ...
    const getById = async(req, res) => {
        ...
        try {
            // Alçada para exibição
            isMatchOrError(uParams && uParams.con_contratos >= 2, `${noAccessMsg} "Exibição de consignatário"`)
        } catch (error) {
            return res.status(401).send(error)
        }
        const tabelaDomain = `${dbPrefix}_${uParams.cliente}_${uParams.dominio}.${tabela}`
        ...
        const ret = app.db({ co: `${tabelaDomain}` })
            .select(`cb.id as id_cad_bancos`, `cb.nome as banco`, `cb.febraban`, `co.id as id_consignatario`, `co.agencia`, `co.qmp`, `co.qmar`)
