...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            existsOrError(body.id_fin_eventos, 'Id do evento da base não informado')
            existsOrError(body.id_fin_base_fixa, 'Nome do evento da base não informado')
        } catch (error) {
            app.api.logger.logError({ log: { line: `Error in file: ${__filename}.${__function} ${error}`, sConsole: true } })
    ...
    const saveByBatch = async (req, res) => {
        ...
        for (let obj in batch) {
            ...
            if (!body.id_fin_base_fixa) bodyValidation.push('Nome do evento da base não informado')
            ...
            if (bodyIdFinEvento && bodyIdFinEventoBase) {
                const excluded = await app.db(tabelaDomain).where({ id_fin_eventos: bodyIdFinEvento.id, id_fin_base_fixa: bodyIdFinEventoBase.id }).delete()
                console.log('excluded:', 'bodyIdFinEvento', bodyIdFinEvento.id_evento, 'bodyIdFinEventoBase', bodyIdFinEventoBase.id);
            }
