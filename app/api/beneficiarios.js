...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            existsOrError(body.id_serv, 'Órgão não informado')
            existsOrError(body.dt_inicio, 'CPF do Trabalhador não informado')
            existsOrError(body.inc_fis_men, 'Incapacidade Física não informada')
            existsOrError(body.dt_inc_fis, 'Data de Nascimento do beneficiário não informada')
    } catch (error) {
        
        console.log(body.id_serv);
            return res.status(400).send(error)
        }
