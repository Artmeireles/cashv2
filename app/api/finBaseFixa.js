...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            existsOrError(body.id_base_fixa, 'Id do evento base não informado')
            existsOrError(body.nome_base_fixa, 'Nome do evento base não informado')
        } catch (error) {
            app.api.logger.logError({ log: { line: `Error in file: ${__filename}.${__function} ${error}`, sConsole: true } })
    ...
    const saveByBatch = async (req, res) => {
        ...
        for (let obj in batch) {
            ...
            if (!body.id_base_fixa) bodyValidation.push('Id do evento base não informado')
            if (!body.nome_base_fixa) bodyValidation.push('Nome do evento base não informado')
            if (bodyValidation.length > 0) batchResult.push({ evento: body.id_base_fixa, result: JSON.stringify(bodyValidation) })

            const bodyEventoBase = await app.db({ fe: tabelaDomain }).where({ id_base_fixa: body.id_base_fixa }).first()
            body.id = (bodyEventoBase && bodyEventoBase.id ? bodyEventoBase.id : null) || undefined
    ...
    const remove = async (req, res) => {
        ...
        const tabelaDomain = `${dbPrefix}_${uParams.cliente}_${uParams.dominio}.${tabela}`
        const tabelaFinBaseFixaEventosDomain = `${dbPrefix}_${uParams.cliente}_${uParams.dominio}.fin_basefixaeventos`
        try {
            let sql = app.db(tabelaFinBaseFixaEventosDomain).count('id', { as: 'count' })
                .where({ id_fin_base_fixa: req.params.id })
            sql = await app.db.raw(sql.toString())
            const count = sql[0][0].count
