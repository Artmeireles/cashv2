...
MySQL - 10.4.21-MariaDB : Database - wwmgca_maribondo_ativos
*********************************************************************
*/
...
/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, wwmgca_maribondo_ativos
CREATE DATABASE /*!32312 IF NOT EXISTS*/`wwmgca_major_ativos` /*!40100 DEFAULT CHARACTER SET utf8mb4 */;
wwmgca_maribondo_ativos
USE `wwmgca_major_ativos`;
...
BEGIN
	...
	-- IF (r_margemC > 0) THEN
		...
		-- END IF;
	-- END IF;
	RETURN JSON_OBJECT('margemFolha', r_margemFinal, 'somaEventos', r_consignadoPos);
    END */$$
DELIMITER ;
...
BEGIN
	...
	SELECT MAX(CAST(fr.ano AS UNSIGNED)) INTO v_ano FROM fin_rubricas fr
		...
		WHERE (cs.id  = v_id OR cs.cpf  = v_id) AND fp.`situacao` = 0 AND fp.mes <=12;
	SELECT lpad(MAX(CAST(fr.mes AS UNSIGNED)),2,'0') INTO v_mes FROM fin_rubricas fr
		JOIN cad_servidores cs ON cs.id = fr.id_cad_servidores
		JOIN fin_parametros fp ON fp.`ano` = fr.ano AND fp.`mes` = fr.mes AND fp.`complementar` = fr.complementar
		WHERE (cs.id  = v_id OR cs.cpf  = v_id) AND fr.ano  = v_ano AND CAST(fr.mes AS UNSIGNED) <= 12;
