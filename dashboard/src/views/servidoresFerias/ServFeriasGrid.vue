...
import ServFeriasForm from './ServFeriasForm.vue';
...
const initFilters = () => {
    filters.value = {
        ...
        dt_i_aquisicao: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
...
<template>
    <div class="card">
        <ServFeriasForm @changed="loadData" v-if="['new', 'edit'].includes(mode)" />
        <DataTable
            ref="dt"
            :value="gridData"
            :paginator="true"
            ...
            filterDisplay="menu"
            :filters="filters"
            :globalFilterFields="['id_serv_vinc', 'dt_i_aquisicao', 'periodo']"
        ...
        >
            ...
            <Column field="dt_i_aquisicao" header="Data Aquisição" sortable>
                <template #body="{ data }">
                    {{ data.dt_i_aquisicao }}
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter" placeholder="Localize por Data Aquisição" />
                </template>
            </Column>
            <Column field="periodo" header="Período" sortable>
                ...
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter" placeholder="Localize por Período" />
                </template>
            </Column>
