...
import ServAfastamentoForm from './ServAfastamentoForm.vue';
...
const initFilters = () => {
    filters.value = {
        ...
        id_par_mtv_af: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
        id_par_tp_acid: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] }
    };
};
// Ref do gridData
const dt = ref(null);
...
<template>
    <div class="card">
        <ServAfastamentoForm @changed="loadData" v-if="['new', 'edit'].includes(mode)" />
        <DataTable
            ref="dt"
            :value="gridData"
            :paginator="true"
            ...
            filterDisplay="menu"
            :filters="filters"
            :globalFilterFields="['id_serv_vinc', 'id_par_mtv_af', 'id_par_tp_acid']"
        ...
        >
            ...
            <Column field="id_par_mtv_af" header="Motivo Afastamento" sortable>
                <template #body="{ data }">
                    {{ data.id_par_mtv_af }}
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter" placeholder="Localize por Motivo Afastamento" />
                </template>
            </Column>
            <Column field="id_par_tp_acid" header="Tipo Acidente" sortable>
                <template #body="{ data }">
                    {{ data.id_par_tp_acid }}
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter" placeholder="Localize por Tipo Acidente" />
                </template>
            </Column>
