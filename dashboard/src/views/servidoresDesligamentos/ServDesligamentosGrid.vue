...
import ServDesligamentoForm from './ServDesligamentoForm.vue';
...
const urlBase = ref(`${baseApiUrl}/serv-desligamentos`);
// Inicializa os filtros
const initFilters = () => {
    filters.value = {
        global: { value: null, matchMode: FilterMatchMode.CONTAINS },
        id_serv_vinc: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
        id_par_mt_dlg: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
        dt_deslig: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] }
    };
};
// Ref do gridData
const dt = ref(null);
...
<template>
    <div class="card">
        <ServDesligamentoForm @changed="loadData" v-if="['new', 'edit'].includes(mode)" />
        <DataTable
            ref="dt"
            :value="gridData"
            :paginator="true"
            ...
            filterDisplay="menu"
            :filters="filters"
            :globalFilterFields="['id_serv_vinc', 'id_par_mt_dlg', 'dt_deslig']"
        ...
        >
            ...
            <Column field="id_par_mt_dlg" header="Motivo Afastamento" sortable>
                <template #body="{ data }">
                    {{ data.id_par_mt_dlg }}
                </template>
                <template #filter="{ filterModel }">
            ...
            <Column field="dt_deslig" header="Tipo Acidente" sortable>
                <template #body="{ data }">
                    {{ data.dt_deslig }}
                </template>
                <template #filter="{ filterModel }">
