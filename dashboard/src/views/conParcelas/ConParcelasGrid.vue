...
import ConParcelaForm from './ConParcelaForm.vue';
...
const urlBase = ref(`${baseApiUrl}/con-parcelas`);
// Inicializa os filtros
const initFilters = () => {
    filters.value = {
        global: { value: null, matchMode: FilterMatchMode.CONTAINS },
        id_con_contratos: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
...
<template>
    <div class="card">
        <ConParcelaForm @changed="loadData" v-if="['new', 'edit'].includes(mode)" />
        <DataTable ref="dt" :value="gridData" :paginator="true" :rowsPerPageOptions="[5, 10, 20, 50]"
            ...
            filterDisplay="menu" :filters="filters" :globalFilterFields="['id_con_contratos', 'parcela', 'vencimento']"
            ...
            <Column field="id_con_contratos" header="Contratos" sortable>
                <template #body="{ data }">
                    {{ data.id_con_contratos }}
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter"
                        placeholder="Localize por Contratos" />
                </template>
            </Column>
            <Column field="parcela" header="Parcelas" sortable>
                <template #body="{ data }">
                    {{ data.parcela }}
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter"
                        placeholder="Localize por Parcelas" />
                </template>
            </Column>
            <Column field="vencimento" header="Vencimento" sortable>
                ...
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter"
                        placeholder="Localize por Vencimento" />
                </template>
            </Column>
