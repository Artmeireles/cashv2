...
const initFilters = () => {
    filters.value = {
        ...
        id_es_envio: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
        codigo: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
        ocorrencia: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] }
    };
};
// Ref do gridData
const dt = ref(null);
...
<template>
    <div class="card">
        <EsRejeicaoForm @changed="loadData" v-if="['new', 'edit'].includes(mode)" />
        <DataTable
            ref="dt"
            :value="gridData"
            :paginator="true"
            ...
            filterDisplay="menu"
            :filters="filters"
            :globalFilterFields="['id_es_envio', 'codigo', 'ocorrencia']"
        ...
        >
            ...
            <Column field="id_es_envio" header="Parâmetros" sortable>
                <template #body="{ data }">
                    {{ data.id_es_envio }}
                </template>
            ...
            <Column field="codigo" header="Lote" sortable>
                <template #body="{ data }">
                    {{ data.codigo }}
                </template>
                <template #filter="{ filterModel }">
            ...
            <Column field="ocorrencia" header="Recibo" sortable>
                <template #body="{ data }">
                    {{ data.ocorrencia }}
                </template>
                <template #filter="{ filterModel }">
