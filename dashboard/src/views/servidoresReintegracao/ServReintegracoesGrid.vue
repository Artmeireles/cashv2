...
import ServReintegracaoForm from './ServReintegracaoForm.vue';
...
const initFilters = () => {
    filters.value = {
        ...
        id_par_tp_rein: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
        nr_proc_jud: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] }
    };
};
// Ref do gridData
const dt = ref(null);
...
<template>
    <div class="card">
        <ServReintegracaoForm @changed="loadData" v-if="['new', 'edit'].includes(mode)" />
        <DataTable
            ref="dt"
            :value="gridData"
            :paginator="true"
            ...
            filterDisplay="menu"
            :filters="filters"
            :globalFilterFields="['id_serv_vinc', 'id_par_tp_rein', 'nr_proc_jud']"
        ...
        >
            ...
            <Column field="id_serv_vinc" header="Servidor" sortable>
                <template #body="{ data }">
                    {{ data.id_serv_vinc }}
                </template>
            ...
            <Column field="id_par_tp_rein" header="Tipo da Reintegração" sortable>
                <template #body="{ data }">
                    {{ data.id_par_tp_rein }}
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter" placeholder="Localize por Tipo da Reintegração" />
                </template>
            </Column>
            <Column field="nr_proc_jud" header="Processo Judicial" sortable>
                <template #body="{ data }">
                    {{ data.nr_proc_jud }}
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter" placeholder="Localize por Processo Judicial" />
                </template>
            </Column>
