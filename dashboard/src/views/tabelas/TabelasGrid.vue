...
import TabelaForm from './TabelaForm.vue';
...
const urlBase = ref(`${baseApiUrl}/tabelas`);
// Inicializa os filtros
const initFilters = () => {
    filters.value = {
        global: { value: null, matchMode: FilterMatchMode.CONTAINS },
        tipo_tabela: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
...
<template>
    <div class="card">
        <TabelaForm @changed="loadData" v-if="['new', 'edit'].includes(mode)" />
        <DataTable ref="dt" :value="gridData" :paginator="true" :rowsPerPageOptions="[5, 10, 20, 50]"
            ...
            filterDisplay="menu" :filters="filters" :globalFilterFields="['tipo_tabela', 'cod_tabela']"
            ...
            <Column field="tipo_tabela" header="Tipo da Tabela" sortable>
                <template #body="{ data }">
                    {{ data.tipo_tabela }}
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter"
                        placeholder="Localize por Tipo da Tabela" />
                </template>
            </Column>
            <Column field="cod_tabela" header="Código da Tabela" sortable>
                <template #body="{ data }">
                    {{ data.cod_tabela }}
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter"
                        placeholder="Localize por Código da Tabela" />
                </template>
            </Column>
