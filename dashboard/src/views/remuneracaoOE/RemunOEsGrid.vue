...
import RemunOEForm from './RemunOEForm.vue';
...
const initFilters = () => {
    filters.value = {
        ...
        id_param_ind_mv: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] }
    };
};
// Ref do gridData
const dt = ref(null);
...
<template>
    <div class="card">
        <RemunOEForm @changed="loadData" v-if="['new', 'edit'].includes(mode)" />
        <DataTable
            ref="dt"
            :value="gridData"
            :paginator="true"
            ...
            filterDisplay="menu"
            :filters="filters"
            :globalFilterFields="['id_serv_vinc', 'nr_insc', 'id_param_ind_mv']"
        ...
        >
            ...
            <Column field="id_param_ind_mv" header="Indicador de Desconto" sortable>
                <template #body="{ data }">
                    {{ data.id_param_ind_mv }}
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" class="p-column-filter" placeholder="Localize por Indicador de Desconto" />
                </template>
            </Column>
